window.onload = function () {
    ...
    let lastScrollY = 0, activeReplyTrigger = null, activeShareDropdown = null, activeShareButton = null;
    ...
    const maxReplyImages = 4, maxReplyMediaAltLength = 1000;
    ...
    // 신고 사유 목록
    const notificationReportReasons = ["다른 회사 제품 도용 신고","실제 존재하지 않는 제품 등록 신고","스펙·원산지 허위 표기 신고","특허 제품 무단 판매 신고","수출입 제한 품목 신고","반복적인 동일 게시물 신고"];
    ...
    const emojiCategoryMeta = {
        ...
        food: { label: "음식 및 음료", sectionTitle: "음식 및 음료", icon: '<svg viewBox="0 0 24 24" aria-hidden="true" class="tweet-modal__emoji-tab-icon"><g><path d="M5 10.5c0-3.59 3.36-6.5 7.5-6.5s7.5 2.91 7.5 6.5v.58a5.42 5.42 0 01-2.08 4.26L16.5 21H8.5l-1.42-5.66A5.42 5.42 0 015 11.08v-.58zm2 0v.58c0 1.08.5 2.08 1.36 2.76l.3.24.95 3.92h5.78l.95-3.92.3-.24a3.42 3.42 0 001.36-2.76v-.58c0-2.48-2.47-4.5-5.5-4.5S7 8.02 7 10.5z"></path></g></svg>' },
        activities: { label: "활동", sectionTitle: "활동", icon: '<svg viewBox="0 0 24 24" aria-hidden="true" class="tweet-modal__emoji-tab-icon"><g><path d="M12 2.25c5.385 0 9.75 4.365 9.75 9.75S17.385 21.75 12 21.75 2.25 17.385 2.25 12 6.615 2.25 12 2.25zm0 1.5A8.25 8.25 0 103.75 12 8.26 8.26 0 0012 3.75zm-4.1 4.5c.27 0 .53.12.71.33l1.94 2.55 3.12-2.29c.36-.27.87-.22 1.18.12l2.83 3.12a.88.88 0 01-.07 1.24.88.88 0 01-1.24-.07l-2.3-2.54-3.16 2.33a.88.88 0 01-1.23-.16L7.2 9.64a.88.88 0 01.7-1.39z"></path></g></svg>' },
        travel: { label: "여행 및 장소", sectionTitle: "여행 및 장소", icon: '<svg viewBox="0 0 24 24" aria-hidden="true" class="tweet-modal__emoji-tab-icon"><g><path d="M12 2.25c-4.142 0-7.5 3.245-7.5 7.248 0 5.207 6.46 11.611 6.735 11.881a1.08 1.08 0 001.53 0c.275-.27 6.735-6.674 6.735-11.881 0-4.003-3.358-7.248-7.5-7.248zm0 17.493c-1.758-1.878-6-6.838-6-10.245 0-3.172 2.686-5.748 6-5.748s6 2.576 6 5.748c0 3.407-4.242 8.367-6 10.245zm0-13.243a3 3 0 100 6 3 3 0 000-6z"></path></g></svg>' },
    ...
    // 최근 사용 이모지 목록을 초기화한다
    function clearRecentEmojis() { try { window.localStorage.removeItem(emojiRecentsKey); } catch { return; } }
    // 이모지 검색 입력값을 소문자로 반환한다
    function getEmojiSearchTerm() { return replyEmojiSearchInput?.value.trim().toLowerCase() ?? ""; }
    ...
    // 미디어 편집 패널 제목을 반환한다
    function getReplyMediaTitle() { return "이미지 설명 수정"; }
    ...
    function saveReplyMediaEdits() {
        ...
        renderReplyAttachment(); syncMediaAltTrigger();
    ...
    function resetReplyAttachment() {
        ...
        attachedReplyFiles = []; pendingAttachmentEditIndex = null;
    ...
    function applyReplyFormat(format) {
        ...
        replyEditor.focus();
        if (!hasReplyEditorText()) { togglePendingReplyFormat(format); syncReplyFormatButtons(); return; }
        ...
        saveReplySelection(); syncReplySubmitState(); syncReplyFormatButtons();
    }
    ...
    function syncReplyFormatButtons() {
        ...
        replyFormatButtons.forEach((btn) => {
            ...
            if (!fmt) return;
            const active = hasReplyEditorText() ? document.queryCommandState(fmt) : pendingReplyFormats.has(fmt);
    ...
    function syncReplySubmitState() {
        if (!replyEditor || !replySubmitButton || !replyProgressBar || !replyProgress) return;
    ...
    function getReplyContextText(tweetItem) {
        return getTextContent(tweetItem?.querySelector(".tweet-reply-to")) || "답글을 보낼 게시물을 찾지 못했습니다.";
    }
    ...
    function populateReplyModal(button) {
        ...
        if (!ti) return;
        if (replyContextButton) replyContextButton.textContent = getReplyContextText(ti);
        if (replySourceAvatar) replySourceAvatar.src = ti.querySelector(".tweet-avatar")?.getAttribute("src") ?? replySourceAvatar.src;
        if (replySourceName) replySourceName.textContent = getTextContent(ti.querySelector(".tweet-displayname"));
        if (replySourceHandle) replySourceHandle.textContent = getTextContent(ti.querySelector(".tweet-handle"));
        if (replySourceTime) replySourceTime.textContent = getTextContent(ti.querySelector(".tweet-time"));
        if (replySourceText) replySourceText.textContent = getTextContent(ti.querySelector(".tweet-text"));
    }
    ...
    function openReplyModal(button) {
        ...
        renderDraftPanel(); renderLocationList(); syncLocationUI();
    ...
    // 브라우저 히스토리에 경로를 추가한다
    function pushSharePath(p) { try { window.history.pushState({}, "", p); } catch { return; } }
    ...
    function openShareDropdown(button) {
        ...
        lc.addEventListener("click", (e) => {
            ...
            if (!ab || !activeShareButton) { e.stopPropagation(); return; }
            e.preventDefault(); e.stopPropagation();
            if (ab.classList.contains("share-menu-item--copy")) { copyShareLink(activeShareButton); return; }
    ...
    function getNotificationDropdownItems(button) {
        ...
        return [
            ...
            { actionClass: "menu-item--block", label: `${handle} 님 차단하기`, testid: "block", icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><g><path d="M12 3.75c-4.55 0-8.25 3.69-8.25 8.25 0 1.92.66 3.68 1.75 5.08L17.09 5.5C15.68 4.4 13.92 3.75 12 3.75zm6.5 3.17L6.92 18.5c1.4 1.1 3.16 1.75 5.08 1.75 4.56 0 8.25-3.69 8.25-8.25 0-1.92-.65-3.68-1.75-5.08zM1.75 12C1.75 6.34 6.34 1.75 12 1.75S22.25 6.34 22.25 12 17.66 22.25 12 22.25 1.75 17.66 1.75 12z"></path></g></svg>' },
            { actionClass: "menu-item--report", label: "게시물 신고하기", testid: "report", icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><g><path d="M3 2h18.61l-3.5 7 3.5 7H5v6H3V2zm2 12h13.38l-2.5-5 2.5-5H5v10z"></path></g></svg>' },
        ];
    }
    ...
    function handleNotificationDropdownAction(button, actionClass) {
        ...
        if (actionClass === "menu-item--block") { openNotificationBlockModal(button); return; }
    ...
    function openNotificationDropdown(button) {
        ...
        const lc = document.createElement("div");
        lc.className = "layers-dropdown-container";
        lc.innerHTML = `<div class="layers-overlay"></div><div class="layers-dropdown-inner"><div role="menu" class="dropdown-menu" style="top: ${top}px; right: ${right}px;"><div><div class="dropdown-inner" data-testid="Dropdown">${items.map((it) => { const ta = it.testid ? ` data-testid="${it.testid}"` : ""; return `<button type="button" role="menuitem" class="menu-item ${it.actionClass}"${ta}><span class="menu-item__icon">${it.icon}</span><span class="menu-item__label">${it.label}</span></button>`; }).join("")}</div></div></div></div>`;
        lc.addEventListener("click", (e) => {
            const item = e.target.closest(".menu-item");
            if (item) {
                e.preventDefault(); e.stopPropagation();
                if (activeMoreButton) { const ac = Array.from(item.classList).find((c) => c.startsWith("menu-item--")) ?? ""; handleNotificationDropdownAction(activeMoreButton, ac); }
                return;
            }
            e.stopPropagation();
        });
    ...
    function deleteSelectedDrafts() {
        ...
        getDraftItems().forEach((i) => { if (draftPanelState.selectedItems.has(i)) i.remove(); });
        exitDraftEditMode();
    }
    ...
    function getDraftEmptyCopy() { return { title: "잠시 생각을 정리합니다", body: "아직 게시할 준비가 되지 않았나요? 임시저장해 두고 나중에 이어서 작성하세요." }; }
    // 임시저장 삭제 확인 문구를 반환한다
    function getDraftConfirmCopy() { return { title: "전송하지 않은 게시물 삭제하기", body: "이 작업은 취소할 수 없으며 선택한 전송하지 않은 게시물이 삭제됩니다." }; }
    ...
    function closeDraftPanel({ restoreFocus = true } = {}) {
        ...
        draftView.hidden = true; composeView.hidden = false;
    ...
    function loadDraftIntoComposer(item) {
        ...
        closeDraftPanel({ restoreFocus: false });
        syncReplySubmitState(); saveReplySelection();
        window.requestAnimationFrame(() => { replyEditor.focus(); });
    }

    // ===== 8. Init & Events =====
    // 초기 UI 상태 설정
    renderLocationList(); syncLocationUI(); syncUserTagTrigger(); setActiveTab("notifications");
    ...
    notifTabs.forEach((tab) => {
        tab.addEventListener("click", (e) => {
            e.preventDefault();
            notifTabs.forEach((t) => { t.classList.remove("notif-tab--active"); t.setAttribute("aria-selected", "false"); });
            tab.classList.add("notif-tab--active"); tab.setAttribute("aria-selected", "true");
        });
    });

    // 좋아요 버튼 클릭 시 활성/비활성을 토글한다
    document.querySelectorAll(".tweet-action-btn--like").forEach((button) => {
        const path = button.querySelector("path"), count = button.querySelector(".tweet-action-count");
    ...
    document.querySelectorAll(".tweet-action-btn--bookmark").forEach((button) => {
        button.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); setBookmarkButtonState(button, !button.classList.contains("active")); });
    });

    // 답글 버튼 클릭 시 답글 모달을 연다
    document.querySelectorAll("[data-testid='reply']").forEach((button) => {
        button.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); closeShareDropdown(); closeNotificationDropdown(); openReplyModal(button); });
    });

    // 더보기(캐럿) 버튼 클릭 시 드롭다운을 토글한다
    document.querySelectorAll("[data-testid='caret']").forEach((button) => {
        button.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); activeMoreButton === button ? closeNotificationDropdown() : openNotificationDropdown(button); });
    });

    // 공유 버튼 클릭 시 공유 드롭다운을 토글한다
    document.querySelectorAll(".tweet-action-btn--share").forEach((button) => {
        button.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); closeNotificationDropdown(); activeShareButton === button ? closeShareDropdown() : openShareDropdown(button); });
    });
    ...
    replyEditor?.addEventListener("keydown", (e) => {
        ...
        e.preventDefault(); applyReplyFormat(key === "b" ? "bold" : "italic");
    });

    // 미디어 업로드 버튼 클릭 시 파일 선택 대화상자를 연다
    replyMediaUploadButton?.addEventListener("click", (e) => {
        e.preventDefault(); pendingAttachmentEditIndex = null;
        if (replyFileInput) replyFileInput.value = "";
    ...
    document.addEventListener("selectionchange", () => {
        ...
        saveReplySelection(); syncReplyFormatButtons();
    });
    ...
    replyEmojiTabs.forEach((tab) => {
        tab.addEventListener("click", () => { const cat = tab.getAttribute("data-emoji-category"); if (cat) { activeEmojiCategory = cat; renderEmojiPicker(); } });
    });

    // 이모지 옵션/지우기 버튼 mousedown 시 포커스 이동을 방지한다
    replyEmojiContent?.addEventListener("mousedown", (e) => { if (e.target.closest(".tweet-modal__emoji-option, .tweet-modal__emoji-clear")) e.preventDefault(); });

    // 이모지 클릭 시 에디터에 삽입하고, 지우기 클릭 시 최근 목록을 초기화한다
    replyEmojiContent?.addEventListener("click", (e) => {
        if (e.target.closest("[data-action='clear-recent']")) { clearRecentEmojis(); activeEmojiCategory = "recent"; renderEmojiPicker(); return; }
    ...
    replyLocationList?.addEventListener("click", (e) => {
        ...
        if (loc) { applyLocation(loc); closeLocationPanel(); }
    });
    ...
    replyTagChipList?.addEventListener("click", (e) => {
        ...
        renderTagChipList(); runTagSearch(); replyTagSearchInput?.focus();
    });
    ...
    replyTagResults?.addEventListener("click", (e) => {
        ...
        renderTagChipList();
        if (replyTagSearchInput) replyTagSearchInput.value = "";
        renderTagResults([]); replyTagSearchInput?.focus();
    });

    // 답글 제출 버튼 클릭 시 답글 수를 증가시키고 모달을 닫는다
    replySubmitButton?.addEventListener("click", () => {
        if (!activeReplyTrigger || replySubmitButton.disabled) return;
        updateReplyCount(activeReplyTrigger); closeReplyModal({ skipConfirm: true });
    });
    ...
    document.addEventListener("keydown", (e) => { if (e.key === "Escape") { closeShareModal(); closeNotificationModal(); closeShareDropdown(); closeNotificationDropdown(); } });
    // 창 크기 변경 시 이모지 피커 위치를 재계산한다
    window.addEventListener("resize", () => { if (replyEmojiPicker && !replyEmojiPicker.hidden) updateEmojiPickerPosition(); }, { passive: true });
    // 스크롤 시 이모지 피커 위치를 재계산한다
    window.addEventListener("scroll", () => { if (replyEmojiPicker && !replyEmojiPicker.hidden) updateEmojiPickerPosition(); }, { passive: true });
