window.addEventListener("load", (e) => {
    ...
    const replyEditor = replyModalOverlay?.querySelector(
        ".tweet-modal__editor",
    );
    const replySubmitButton = replyModalOverlay?.querySelector(
        ".tweet-modal__submit",
    );
    const replyProgress = replyModalOverlay?.querySelector(
        ".tweet-modal__progress",
    );
    const replyProgressBar = replyModalOverlay?.querySelector(
        ".tweet-modal__progress-bar",
    );
    const replyContextButton = replyModalOverlay?.querySelector(
        ".tweet-modal__context-button",
    );
    const replyFooterMeta = replyModalOverlay?.querySelector(
        ".tweet-modal__footer-meta",
    );
    const replySourceAvatar = replyModalOverlay?.querySelector(
        ".tweet-modal__source-avatar",
    );
    const replySourceName = replyModalOverlay?.querySelector(
        ".tweet-modal__source-name",
    );
    const replySourceHandle = replyModalOverlay?.querySelector(
        ".tweet-modal__source-handle",
    );
    const replySourceTime = replyModalOverlay?.querySelector(
        ".tweet-modal__source-time",
    );
    const replySourceText = replyModalOverlay?.querySelector(
        ".tweet-modal__source-text",
    );
    const replyFormatButtons =
        replyModalOverlay?.querySelectorAll("[data-format]") ?? [];
    const replyEmojiButton = replyModalOverlay?.querySelector(
        "[data-testid='emojiButton']",
    );
    const replyEmojiPicker = replyModalOverlay?.querySelector(
        ".tweet-modal__emoji-picker",
    );
    const replyEmojiSearchInput = replyModalOverlay?.querySelector(
        "[data-testid='emojiSearchInput']",
    );
    const replyEmojiTabs =
        replyModalOverlay?.querySelectorAll(".tweet-modal__emoji-tab") ?? [];
    const replyEmojiContent = replyModalOverlay?.querySelector(
        "[data-testid='emojiPickerContent']",
    );
    const replyMediaUploadButton = replyModalOverlay?.querySelector(
        "[data-testid='mediaUploadButton']",
    );
    const replyFileInput = replyModalOverlay?.querySelector(
        "[data-testid='fileInput']",
    );
    const replyAttachmentPreview = replyModalOverlay?.querySelector(
        "[data-attachment-preview]",
    );
    const replyAttachmentMedia = replyModalOverlay?.querySelector(
        "[data-attachment-media]",
    );
    const composeView = replyModalOverlay?.querySelector(
        ".tweet-modal__compose-view",
    );
    const replyGeoButton = replyModalOverlay?.querySelector(
        "[data-testid='geoButton']",
    );
    const replyGeoButtonPath = replyGeoButton?.querySelector("path");
    const replyLocationDisplayButton = replyModalOverlay?.querySelector(
        "[data-location-display]",
    );
    const replyLocationName = replyModalOverlay?.querySelector(
        "[data-location-name]",
    );
    const replyLocationView = replyModalOverlay?.querySelector(
        ".tweet-modal__location-view",
    );
    const replyLocationCloseButton = replyModalOverlay?.querySelector(
        ".tweet-modal__location-close",
    );
    const replyLocationDeleteButton = replyModalOverlay?.querySelector(
        "[data-location-delete]",
    );
    const replyLocationCompleteButton = replyModalOverlay?.querySelector(
        "[data-location-complete]",
    );
    const replyLocationSearchInput = replyModalOverlay?.querySelector(
        "[data-location-search]",
    );
    const replyLocationList = replyModalOverlay?.querySelector(
        "[data-location-list]",
    );
    const replyUserTagTrigger = replyModalOverlay?.querySelector(
        "[data-user-tag-trigger]",
    );
    const replyUserTagLabel = replyModalOverlay?.querySelector(
        "[data-user-tag-label]",
    );
    const replyTagView = replyModalOverlay?.querySelector(
        ".tweet-modal__tag-view",
    );
    const replyTagCloseButton = replyModalOverlay?.querySelector(
        "[data-testid='tag-back']",
    );
    const replyTagCompleteButton = replyModalOverlay?.querySelector(
        "[data-tag-complete]",
    );
    const replyTagSearchForm = replyModalOverlay?.querySelector(
        "[data-tag-search-form]",
    );
    const replyTagSearchInput =
        replyModalOverlay?.querySelector("[data-tag-search]");
    const replyTagChipList = replyModalOverlay?.querySelector(
        "[data-tag-chip-list]",
    );
    const replyTagResults =
        replyModalOverlay?.querySelector("[data-tag-results]");
    const replyMediaAltTrigger = replyModalOverlay?.querySelector(
        "[data-media-alt-trigger]",
    );
    const replyMediaAltLabel = replyModalOverlay?.querySelector(
        "[data-media-alt-label]",
    );
    const replyMediaView = replyModalOverlay?.querySelector(
        ".tweet-modal__media-view",
    );
    const replyMediaBackButton = replyModalOverlay?.querySelector(
        "[data-testid='media-back']",
    );
    const replyMediaPrevButton =
        replyModalOverlay?.querySelector("[data-media-prev]");
    const replyMediaNextButton =
        replyModalOverlay?.querySelector("[data-media-next]");
    const replyMediaSaveButton =
        replyModalOverlay?.querySelector("[data-media-save]");
    const replyMediaTitle =
        replyModalOverlay?.querySelector("[data-media-title]");
    const replyMediaPreviewImages =
        replyModalOverlay?.querySelectorAll("[data-media-preview-image]") ?? [];
    const replyMediaAltInput = replyModalOverlay?.querySelector(
        "[data-media-alt-input]",
    );
    const replyMediaAltCount = replyModalOverlay?.querySelector(
        "[data-media-alt-count]",
    );
        ...
        return Array.from(tweetItems)
            .map((tweetItem, index) => {
                const name = getTextContent(
                    tweetItem.querySelector(".tweet-displayname"),
                );
                const handle = getTextContent(
                    tweetItem.querySelector(".tweet-handle"),
                );
                const avatar =
                    tweetItem
                        .querySelector(".tweet-avatar")
                        ?.getAttribute("src") ?? "";
    ...
    function getFilteredLocations() {
        ...
        return cachedLocationNames.filter((location) =>
    ...
    function clearAttachedReplyFileUrls() {
        ...
        attachedReplyFileUrls.forEach((fileUrl) =>
    ...
    function renderReplyAttachment() {
        ...
        const fileGroup = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "g",
        );
    ...
    function removeReplyAttachment(index) {
        attachedReplyFiles = attachedReplyFiles.filter(
    ...
    function handleReplyFileChange(e) {
        ...
        const pickedVideo = nextFiles.find((file) =>
            file.type.startsWith("video/"),
        );
        const pickedImages = nextFiles.filter((file) =>
        ...
        if (pendingAttachmentEditIndex !== null) {
            ...
            if (replacementFile.type.startsWith("video/")) {
                ...
                const editableFiles = isReplyVideoSet()
                    ? []
                    : [...attachedReplyFiles];
        ...
        if (pickedImages.length > 0) {
            ...
            attachedReplyFiles = [...baseImages, ...pickedImages].slice(
                0,
                maxReplyImages,
            );
            renderReplyAttachment();
    ...
    function getReplyContextText(tweetItem) {
        ...
        return (
            getTextContent(replyTo) || "답글을 보낼 게시물을 찾지 못했습니다."
        );
    }
    ...
    function populateReplyModal(button) {
        ...
        if (replySourceAvatar) {
            replySourceAvatar.src =
                tweetItem.querySelector(".tweet-avatar")?.getAttribute("src") ??
                replySourceAvatar.src;
        }
        ...
        if (replySourceTime) {
            replySourceTime.textContent = getTextContent(
                tweetItem.querySelector(".tweet-time"),
            );
        }

        if (replySourceText) {
            replySourceText.textContent = getTextContent(
                tweetItem.querySelector(".tweet-text"),
            );
        }
    }
    ...
    function updateReplyCount(button) {
        ...
        const currentCount =
            Number.parseInt(countElement.textContent || "0", 10) || 0;
    ...
    // ===== 6. 공유/더보기 드롭다운 =====
    // 공유 메뉴/모달은 버튼 기준으로 열고, 주소와 토스트도 여기서 함께 관리한다.
    function pushSharePath(pathname) {
        try {
    ...
    function getSharePostMeta(button) {
        const tweetItem = button.closest(".notif-tweet-item");
        const tweetItems = Array.from(
            document.querySelectorAll(".notif-tweet-item"),
        );
        const handle =
            getTextContent(tweetItem?.querySelector(".tweet-handle")) ||
            "@sokkomann";
        const bookmarkButton =
            tweetItem?.querySelector(".tweet-action-btn--bookmark") ?? null;
        const tweetId = String(Math.max(tweetItems.indexOf(tweetItem), 0) + 1);
        const url = new URL(window.location.href);

        url.pathname = `/${handle.replace("@", "") || "user"}/status/${tweetId}`;
        url.hash = "";
        url.search = "";
    ...
    function getShareUserRows() {
        ...
        if (users.length === 0) {
            return `
                <div class="share-sheet__empty">
                    <p>전송할 수 있는 사용자가 없습니다.</p>
                </div>
            `;
        }
        ...
        return users
            .map(
                (user) => `
                    <button type="button" class="share-sheet__user" data-share-user-id="${escapeHtml(user.id)}">
    ...
    function getNotificationDropdownItems(button) {
        ...
        const handle =
            getTextContent(tweetItem?.querySelector(".tweet-handle")) ||
            "@sokkomann";
        const isFollowing = notificationFollowState.get(handle) ?? false;
    ...
    // 드롭다운/모달에서 공통으로 쓸 계정 정보는 현재 알림 카드 DOM에서 읽는다.
    function getNotificationUserMeta(button) {
        const tweetItem = button.closest(".notif-tweet-item");
        const allTweetItems = Array.from(
            document.querySelectorAll(".notif-tweet-item"),
        );
        const handle =
            getTextContent(tweetItem?.querySelector(".tweet-handle")) ||
            "@sokkomann";
        const displayName =
            getTextContent(tweetItem?.querySelector(".tweet-displayname")) ||
            "사용자";
        const tweetId = String(
            Math.max(allTweetItems.indexOf(tweetItem), 0) + 1,
        );
    ...
    // 리스트/차단/신고 모달은 한 번에 하나만 열리도록 유지한다.
    ...
    function openNotificationBlockModal(button) {
        ...
        <div class="notification-dialog__card notification-dialog__card--small" role="alertdialog" aria-modal="true" aria-labelledby="notification-block-title" aria-describedby="notification-block-desc">
            <h2 id="notification-block-title" class="notification-dialog__title">${handle} 님을 차단할까요?</h2>
            <p id="notification-block-desc" class="notification-dialog__description">
                내 공개 게시물을 볼 수 있지만 더 이상 게시물에 참여할 수 없습니다. 또한 ${handle} 님은 나를 팔로우하거나 쪽지를 보낼 수 없으며, 이 계정과 관련된 알림도 내게 표시되지 않습니다.
            </p>
    ...
    function openNotificationReportModal(button) {
        ...
        modal.innerHTML = `
        <div class="notification-dialog__backdrop"></div>
        <div class="notification-dialog__card notification-dialog__card--report" role="dialog" aria-modal="true" aria-labelledby="notification-report-title">
            <div class="notification-dialog__header">
                ...
                <h2 id="notification-report-title" class="notification-dialog__title">신고하기</h2>
            </div>
    ...
    function handleNotificationDropdownAction(button, actionClass) {
        ...
        if (actionClass === "menu-item--follow-toggle") {
            ...
            notificationFollowState.set(handle, nextFollowing);
            closeNotificationDropdown();

            if (nextFollowing) {
                showNotificationToast(`${handle} 님을 팔로우함`);
            }
            ...
            <div role="menu" class="dropdown-menu" style="top: ${top}px; right: ${right}px;">
                <div>
                    <div class="dropdown-inner" data-testid="Dropdown">
                        ${items
                            .map((item) => {
                                const testidAttr = item.testid
                                    ? ` data-testid="${item.testid}"`
                                    : "";

                                return `
                                    <button type="button" role="menuitem" class="menu-item ${item.actionClass}"${testidAttr}>
    ...
    `;
        ...
        layerContainer.addEventListener("click", (e) => {
            ...
            if (item) {
                ...
                if (activeMoreButton) {
                    const actionClass =
                        Array.from(item.classList).find((className) =>
                            className.startsWith("menu-item--"),
                        ) ?? "";

                    handleNotificationDropdownAction(
                        activeMoreButton,
                        actionClass,
                    );
                }
    ...
    replyAttachmentMedia?.addEventListener("click", (e) => {
        const removeButton = e.target.closest("[data-attachment-remove-index]");
        ...
        const editButton = e.target.closest("[data-attachment-edit-index]");
    ...
    replyEmojiSearchInput?.addEventListener("input", (e) => {
        renderEmojiPickerContent();
    });

    replyLocationSearchInput?.addEventListener("input", (e) => {
        renderLocationList();
    });

    replyTagSearchForm?.addEventListener("submit", (e) => {
        e.preventDefault();
    });

    replyTagSearchInput?.addEventListener("input", (e) => {
        runTagSearch();
    });
    ...
    replyMediaAltInput?.addEventListener("input", (e) => {
        ...
        currentEdit.alt = replyMediaAltInput.value.slice(
            0,
    ...
    replyEmojiContent?.addEventListener("click", (e) => {
        const clearButton = e.target.closest("[data-action='clear-recent']");
        ...
        const emojiButton = e.target.closest(".tweet-modal__emoji-option");
    ...
    replyLocationCloseButton?.addEventListener("click", (e) => {
        closeLocationPanel();
    });
    ...
    replyLocationDeleteButton?.addEventListener("click", (e) => {
        resetLocationState();
        closeLocationPanel();
    });

    replyLocationCompleteButton?.addEventListener("click", (e) => {
        if (!pendingLocation) {
            return;
        }
    ...
    replyLocationList?.addEventListener("click", (e) => {
        const locationButton = e.target.closest(".tweet-modal__location-item");
    ...
    replyTagChipList?.addEventListener("click", (e) => {
        const chipButton = e.target.closest("[data-tag-remove-id]");
    ...
    replyTagResults?.addEventListener("click", (e) => {
        const userButton = e.target.closest("[data-tag-user-id]");
    ...
    const draftView = replyModalOverlay?.querySelector(
        ".tweet-modal__draft-view",
    );
    ...
    function areAllDraftItemsSelected() {
        ...
        return (
            items.length > 0 &&
            items.every((item) => draftPanelState.selectedItems.has(item))
        );
    }
